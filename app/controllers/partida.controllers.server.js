...
module.exports.createPartida = function(req, res, next){
	...
	var dados = req.body
	if(dados.data != "" && dados.player01 != "" && dados.player02 != "") {
		var partida = new Partida()
		partida.data = dados.data
			...
			if(jogador != null) {
				...
				Jogador.findOne({"apelido":dados.player02}, function(err, jogador2){
					...
					if(jogador2 != null) {
						...
						res.write("Falha ao registrar partida!")
						res.write("<p><a href='/partida'> <button class='btn btn-default'> Voltar </button></a></p>")
						res.end()
					}
					}
		...
		if(err){
			...
			if(err){
				...
				if(err){
					...
					res.write("<p>Partida atualizada com sucesso! </p>")
					res.write("<p><a href='/partida/" + id + "'> <button class='btn btn-default'> Voltar </button> </a></p>")
					res.end()
				}
				})
			}
